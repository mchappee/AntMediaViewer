...
+ (void)registerHandler:(Class)handlerClass
{
	...
	for (i = 0; i < count; i++)
	{
		if ([(Class)handlerClass priority] >= [(Class)[registeredHandlers objectAtIndex:i] priority])
		{
			break;
		}
	}
...
- (void)startResponse
{
	...
	CFHTTPMessageSetBody(
		response,
                         (__bridge CFDataRef)UIImagePNGRepresentation(ad.screenshot));
